...
public:
       ...
       // TODO: #8 You may reduce these two function by using Invoke
        ...
        {
                if( m_SubPages[i] )
                        if( (pTmp = m_SubPages[i]->FirstThat(fn, level+1, stD::forward<Args>(pExtra)... ) )
                                return pTmp;
                if( std::invoke(fn, m_Keys[i], level, std::forward<Args>(pExtra)... ) )
                        return &m_Keys[i];
        }
        if( m_SubPages[m_KeyCount] )
                if( (pTmp = m_SubPages[m_KeyCount]->FirstThat(fn, level+1, std::forward<Args>(pExtra)... ) ))
                        return pTmp;
        return 0;
        }
...
void CBTreePage<Trait>::ForEach(F lpfn, size_t level, Args... pExtra)
{
       ...
       {
               if( m_SubPages[i] )
                       m_SubPages[i]->ForEach(lpfn, level+1, std::forward<Args>(pExtra)...);
        //        lpfn(m_Keys[i], level, pExtra1);
               std::invoke(lpfn, m_Keys[i], level, std::forward<Args>(pExtra)...);
       }
       if( m_SubPages[m_KeyCount] )
               m_SubPages[m_KeyCount]->ForEach(lpfn, level+1, std::forward<Args>(pExtra)...);
}
